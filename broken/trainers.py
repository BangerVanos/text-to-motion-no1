...
class Logger:
    def __init__(self, log_dir, project_name="my_project"):
        # Инициализация wandb в оффлайн-режиме.
        wandb.init(project=project_name, dir=log_dir, mode="offline")
...
class CompTrainerV6(object):
    ...
    def __init__(self, args, text_enc, seq_pri, seq_dec, att_layer, mov_dec, mov_enc=None, seq_post=None):
        ...
        self.eval_wrapper = EvaluatorModelWrapper(args)
    ...
    def train(self, train_dataset, val_dataset, plot_eval):
        ...
        self.opt_text_enc = torch.optim.Adam(self.text_enc.parameters(), lr=self.opt.lr)
        self.opt_seq_post = torch.optim.Adam(self.seq_post.parameters(), lr=self.opt.lr)
        self.opt_seq_pri = torch.optim.Adam(self.seq_pri.parameters(), lr=self.opt.lr)
        self.opt_att_layer = torch.optim.Adam(self.att_layer.parameters(), lr=self.opt.lr)
        self.opt_seq_dec = torch.optim.Adam(self.seq_dec.parameters(), lr=self.opt.lr)
        self.opt_mov_dec = torch.optim.Adam(self.mov_dec.parameters(), lr=self.opt.lr * 0.1)

        epoch = 0
        it = 0
        schedule_len = 10 if self.opt.dataset_name == 't2m' else 6
        sub_ep = 0
        ...
        while invalid:
            # Обновляем максимальную длину для датасетов
            train_dataset.reset_max_len(schedule_len * self.opt.unit_length)
            val_dataset.reset_max_len(schedule_len * self.opt.unit_length)

            train_loader = DataLoader(train_dataset, batch_size=self.opt.batch_size, drop_last=True,
                                        num_workers=4, shuffle=True, collate_fn=collate_fn, pin_memory=True)
            val_loader = DataLoader(val_dataset, batch_size=self.opt.batch_size, drop_last=True,
                                    num_workers=4, shuffle=True, collate_fn=collate_fn, pin_memory=True)
            ...
            for sub_epoch in range(sub_ep, self.opt.max_sub_epoch):
                ...
                for i, batch_data in enumerate(train_loader):
                    # Выполняем forward-проход, который задаёт:
                    # self.motions, self.fake_motions и self.caption – необходимые для метрик
                    ...
                    if it % self.opt.log_every == 0:
                        mean_loss = OrderedDict({'val_loss': val_loss})
                        # Логгирование тренировочных метрик
                        self.logger.scalar_summary('val_loss', val_loss, it)
                        ...
                        for tag, value in logs.items():
                            ...
                            mean_loss[tag] = value / self.opt.log_every
                        logs = OrderedDict()
                        print_current_loss(start_time, it, mean_loss, epoch,
                                            sub_epoch=sub_epoch, inner_iter=i,
                                            tf_ratio=tf_ratio, sl_steps=schedule_len)

                    if it % self.opt.save_latest == 0:
                        self.save(pjoin(self.opt.model_dir, 'latest.tar'), epoch, it, sub_ep, schedule_len)
                    time1 = time.time()
                ...
                if epoch % self.opt.save_every_e == 0:
                    self.save(pjoin(self.opt.model_dir, 'E%03d_SE%02d_SL%02d.tar' %
                                    (epoch, sub_ep, schedule_len)), epoch, total_it=it, sub_ep=sub_ep, sl_len=schedule_len)
                ...
                with torch.no_grad():
                    ...
                    for i, batch_data in enumerate(val_loader):
                        ...
                        all_pos_ohot.append(pos_ohot)
                        all_cap_lens.append(cap_lens)
                        ...
                        loss_gen += self.loss_gen.item()

                        # Сбор данных для метрик
                        fake_motions = self.fake_motions.detach().cpu().numpy()
                        real_motions = motions.numpy()
                        all_real_motions.append(real_motions)
                        all_fake_motions.append(fake_motions)

                        # Multimodality: 5 реплик
                        mm_replications = 5
                        batch_mm_embs = []
                        for _ in range(mm_replications):
                            self.forward(batch_data, 0, schedule_len, eval_mode=True)
                            fake_motions = self.fake_motions.detach()
                            
                            # 1. Проверка и исправление m_lens
                            fixed_m_lens = m_lens.to(self.device)
                            fixed_m_lens = torch.div(fixed_m_lens, self.opt.unit_length, rounding_mode='floor')
                            fixed_m_lens = torch.clamp(fixed_m_lens, min=1)  # Гарантируем минимум 1
                            
                            # 2. Проверка NaN/Inf
                            if torch.isnan(fake_motions).any() or torch.isinf(fake_motions).any():
                                print("Обнаружены NaN/Inf в fake_motions, пропускаем батч")
                                continue
                            
                            # 3. Выравнивание размерностей
                            try:
                                motion_emb = self.eval_wrapper.get_motion_embeddings(fake_motions, fixed_m_lens)
                                batch_mm_embs.append(motion_emb.unsqueeze(1))
                            except Exception as e:
                                print(f"Ошибка в get_motion_embeddings: {str(e)}")
                                print(f"Размерности: motions={fake_motions.shape}, m_lens={fixed_m_lens}")
                                continue
                        
                        if batch_mm_embs:
                            mm_motion_embeddings.append(torch.cat(batch_mm_embs, dim=1).cpu().numpy())

                # Усреднение лоссов
                n_val_batches = len(val_loader)
                loss_mot_rec /= n_val_batches
                loss_mov_rec /= n_val_batches
                loss_kld /= n_val_batches
                loss_gen /= n_val_batches

                # Логирование лоссов
                self.logger.scalar_summary('val/loss_mot_rec', loss_mot_rec, it)
                self.logger.scalar_summary('val/loss_mov_rec', loss_mov_rec, it)
                self.logger.scalar_summary('val/loss_kld', loss_kld, it)
                self.logger.scalar_summary('val/loss_gen', loss_gen, it)

                # Подготовка данных для метрик
                word_embs = torch.cat(all_word_embs).to(self.device)
                pos_ohot = torch.cat(all_pos_ohot).to(self.device)
                cap_lens = torch.cat(all_cap_lens)
                m_lens = torch.cat(all_m_lens)
                real_motions = torch.tensor(np.concatenate(all_real_motions), device=self.device)
                fake_motions = torch.tensor(np.concatenate(all_fake_motions), device=self.device)

                # Вычисление эмбеддингов через EvaluatorModelWrapper
                text_embs, real_motion_embs = self.eval_wrapper.get_co_embeddings(
                    word_embs, pos_ohot, cap_lens, real_motions, m_lens
                )
                fake_motion_embs = self.eval_wrapper.get_motion_embeddings(fake_motions, m_lens)

                # Конвертация в numpy
                text_embs = text_embs.cpu().numpy()
                real_motion_embs = real_motion_embs.cpu().numpy()
                fake_motion_embs = fake_motion_embs.cpu().numpy()

                # Вычисление метрик
                # 1. R-Precision
                dist_mat = euclidean_distance_matrix(text_embs, fake_motion_embs)
                argmax = np.argsort(dist_mat, axis=1)
                top_k_mat = calculate_top_k(argmax, top_k=3)
                r_precision = top_k_mat.mean(axis=0)
                r1, r2, r3 = r_precision[0], r_precision[1], r_precision[2]

                # 2. FID
                mu_real, sigma_real = calculate_activation_statistics(real_motion_embs)
                mu_fake, sigma_fake = calculate_activation_statistics(fake_motion_embs)
                fid_value = calculate_frechet_distance(mu_real, sigma_real, mu_fake, sigma_fake)

                # 3. Diversity
                diversity = calculate_diversity(fake_motion_embs, diversity_times=300)

                # 4. Multimodality
                multimodality = 0.0
                if mm_motion_embeddings:
                    mm_motion_embeddings = np.concatenate(mm_motion_embeddings, axis=0)
                    multimodality = calculate_multimodality(mm_motion_embeddings, multimodality_times=100)

                # 5. MM-Dist
                mm_dist = calculate_matching_score(text_embs, fake_motion_embs).mean()

                # Логирование метрик
                metrics = {
                    'val/R_precision_top1': r1,
                    'val/R_precision_top2': r2,
                    'val/R_precision_top3': r3,
                    'val/FID': fid_value,
                    'val/Diversity': diversity,
                    'val/Multimodality': multimodality,
                    'val/MM_Dist': mm_dist
                }
                for name, value in metrics.items():
                    self.logger.scalar_summary(name, value, it)

                print(
                    f'[VAL] Ep: {epoch} | '
                    f'Mot: {loss_mot_rec:.3f} | '
                    f'Mov: {loss_mov_rec:.3f} | '
                    f'KLD: {loss_kld:.3f} | '
                    f'FID: {fid_value:.2f} | '
                    f'R1: {r1:.3f} | '
                    f'Div: {diversity:.3f} | '
                    f'MM: {multimodality:.3f}'
                )
